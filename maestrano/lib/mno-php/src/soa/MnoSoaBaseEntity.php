...
{                   
    ...
    */
    ...
    {            
      ...
      if ( $status != 200 ) {
            MnoSoaLogger::error(__FUNCTION__ . " Error: call to URL $url failed with status $status, response $response, curl_error " . curl_error($curl) . ", curl_errno " . curl_errno($curl), 0);
            curl_close($curl);
            return null;
      }
